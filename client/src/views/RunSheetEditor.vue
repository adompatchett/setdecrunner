...
const loadPeople = async () => {
  try { people.value = await api.get('/people', { limit: 100 }); }
...
async function togglePost(key, ev) {
  ...
  try {
    await api.patch(`/runsheets/${rs.value._id}`, {
      postLocation: next,
...
async function clearPostPlace() {
  ...
  rs.value.postPlace = null;
  try {
    await api.patch(`/runsheets/${rs.value._id}`, { postPlace: null });
    stamp();
  } catch (e) {
...
async function uploadItemPhotos(item, e) {
  try {
    ...
    await api.post(`/items/${item._id}/photos`, fd, {
...
async function updateAttachedQuantity(ai, val) {
  ...
  try {
    ...
    await api.patch(`/runsheets/${id}/items/${ai._id}`, { quantity: q });
    await refreshAttachedItems();
  } catch (e) {
...
async function createAndAddItem() {
  ...
  try {
    ...
    if (newItem.value.file) {
      ...
      await api.post(`/items/${created._id}/photos`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
    }
...
const searchCompletedUsers = async () => {
  try { cbResults.value = await api.get('/users', { q: cbSearch.value }); }
...
const hydratePostPlace = async () => {
  if (rs.value && rs.value.postPlace && typeof rs.value.postPlace === 'string') {
    try { rs.value.postPlace = await api.get(`/places/${rs.value.postPlace}`); } catch (e) {}
  }
};
...
const load = async () => {
  try {
    ...
    if (rs.value.set && typeof rs.value.set === 'string') {
      try { rs.value.set = await api.get(`/sets/${rs.value.set}`); } catch (e) {}
    }
...
const uploadReceipts = async (e) => {
  ...
  try {
    const resp = await api.post(`/runsheets/${rs.value._id}/receipts`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
...
async function refreshAttachedItems () {
  if (!rs.value || !rs.value._id) return;
  try {
    const data = await api.get(`/runsheets/${rs.value._id}/items`);
...
async function searchItems() {
  ...
  try {
    ...
    itemResults.value = await api.get('/items', params);
  } catch (e) {
...
async function addItemToRunsheet(itemOrId, qty = 1) {
  ...
  if (!id || !itemId) return;
  try {
    const resp = await api.post(`/runsheets/${id}/items`, { itemId, quantity: qty || 1 });
...
async function removeAttachedItem(itemId) {
  ...
  if (!id || !itemId) return;
  try {
    const resp = await api.del(`/runsheets/${id}/items/${itemId}`);
    if (Array.isArray(resp)) {
      attachedItems.value = resp;
      rs.value.items = resp;
    } else if (resp && resp.items) {
...
async function createAndAddItemToStop(_stopId_ignored) {
  ...
  try {
    ...
    const created = await api.post('/items', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    await addItemToRunsheet(created._id, newItem.value.qty || 1);
    itemResults.value.unshift(created);
    newItem.value = { name: '', description: '', qty: 1, file: null, preview: '' };
  } catch (e) {
...
const searchUsers = async () => {
  try { userResults.value = await api.get('/users', { q: userSearch.value }); }
  catch (e) { error.value = e?.response?.data?.error || 'Failed to search users'; }
};
const assign = async (u) => {
  try { rs.value = await api.post(`/runsheets/${rs.value._id}/assign`, { userId: u._id }); stamp(); }
  catch (e) { error.value = e?.response?.data?.error || 'Failed to assign'; }
};

/* ============================ Destination (Place) ============================ */
const setTakeTo = async (place) => {
  try { rs.value = await api.patch(`/runsheets/${rs.value._id}`, { takeTo: place._id || place }); stamp(); }
  catch (e) { error.value = e?.response?.data?.error || 'Failed to set destination'; }
};
const clearTakeTo = async () => {
  try { rs.value = await api.patch(`/runsheets/${rs.value._id}`, { takeTo: null }); stamp(); }
...
const searchSets = async () => {
  try { setResults.value = await api.get('/sets', setSearch.value ? { q: setSearch.value } : undefined); }
  catch (e) { error.value = e?.response?.data?.error || 'Failed to search sets'; }
};
const chooseSet = async (s) => {
  try { rs.value = await api.patch(`/runsheets/${rs.value._id}`, { set: s._id }); rs.value.set = s; stamp(); }
  catch (e) { error.value = e?.response?.data?.error || 'Failed to link set'; }
};
const clearSet = async () => {
  try { rs.value = await api.patch(`/runsheets/${rs.value._id}`, { set: null }); rs.value.set = null; stamp(); }
...
async function fetchSuppliers() {
  ...
  try { const q = (search.value || '').trim(); suppliers.value = await api.get('/suppliers', { q }); }
...
async function persistSupplier(idOrNull) {
  if (!rs.value || !rs.value._id) return;
  saving.value = true;
  try { rs.value = await api.patch(`/runsheets/${rs.value._id}`, { supplier: idOrNull }); }
...
async function createSupplier() {
  ...
  creating.value = true;
  try {
    const s = await api.post('/suppliers', body);
  ...
  try {
    ...
    if (rs.value && rs.value.supplier) {
      selectedSupplier.value = typeof rs.value.supplier === 'object'
        ? rs.value.supplier
        : await api.get(`/suppliers/${rs.value.supplier}`).catch(() => null);
    }
  } catch (e) {
