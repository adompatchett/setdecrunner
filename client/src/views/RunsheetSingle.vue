...
<template>
  ...
  /* --------- company block (from external file) --------- */
  const companyAddrLines = computed(() => {
    const raw = COMPANY.companyAddress || '';
    const parts = raw.split(/\n|,/).map(s => s.trim()).filter(Boolean);
    return [parts[0] || '', parts[1] || '', parts[2] || ''];
  });
  const companyBlock = computed(() => ({
    name: COMPANY.companyName || ' ',
    showTitle: !!COMPANY.companyProduction,
    title: COMPANY.companyProduction || '',
  ...
  const supplierAddressLines = computed(() => {
    const raw = supplierObj.value?.address || '';
    const parts = raw.split(/\n|,/).map(s => s.trim()).filter(Boolean);
    return [parts[0] || '', parts[1] || '', parts[2] || ''];
  });
  ...
  const itemsFlat = computed(() => {
    ...
    (rs.value?.items || []).forEach(it => {
      out.push({ item: `${it.name || 'Item'}${it.qty ? ' × ' + it.qty : ''}`, desc: it.description || '' });
    });
    return out;
  });
  const MIN_ROWS = 14;
  ...
  async function hydrateContactPerson() {
    ...
    if (!v) return;
    if (typeof v === 'string') {
      try { contactPerson.value = await api.get(`/people/${v}`); }
  ...
  const money = (n) => (typeof n === 'number' && isFinite(n)) ? new Intl.NumberFormat(undefined,{style:'currency',currency:'USD',maximumFractionDigits:2}).format(n) : '';
  ...
  onMounted(async () => {
    ...
    const data = await api.get(`/runsheets/${route.params.id}`);
    rs.value = data;
  
    // hydrate supplier/takeTo/set if they came back as ids
    if (rs.value?.supplier && typeof rs.value.supplier === 'string') {
      try { rs.value.supplier = await api.get(`/suppliers/${rs.value.supplier}`); } catch {}
    }
    if (rs.value?.takeTo && typeof rs.value.takeTo === 'string') {
      try { rs.value.takeTo = await api.get(`/places/${rs.value.takeTo}`); } catch {}
    }
    if (rs.value?.set && typeof rs.value.set === 'string') {
      try { rs.value.set = await api.get(`/sets/${rs.value.set}`); } catch {}
    }
  ...
  /* --------- PRINT AS 1-PAGE IMAGE (wider + bigger) --------- */
  async function doPrintImage() {
    const el = sheetEl.value;
    if (!el) return;
  
    // Tweak these:
    const ORIENTATION = 'landscape'; // 'portrait' | 'landscape' (landscape is wider)
    const FONT_BOOST = 1.35;         // 1.00 = normal; 1.35 = +35% bigger text
    const PAGE_MARGIN_IN = 0.15;     // small printable margin
    const PRINT_DPI = 96;            // CSS px per inch (browser standard)
    const H2C_SCALE = Math.min(3, window.devicePixelRatio || 2); // crispness
    ...
    // Force capture width to match target page inner width (so it stretches edge-to-edge)
    const targetPxW = Math.round(innerWIn * PRINT_DPI);
  
    // Temporarily adjust the live sheet for capture
    const prev = {
    ...
    try {
      ...
      // Compute display size so we max out width, but never overflow height
