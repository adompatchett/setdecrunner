<template>
    <div>
      ...
      <div class="container">
        ...
        <div class="field">
          ...
          <div v-if="s.photo" style="display:flex;align-items:center;gap:12px;">
            <img :src="photoUrl" alt="Person photo" style="width:96px;height:96px;object-fit:cover;border-radius:8px;" />
  ...
  const load = async () => {
    ...
    try {
      s.value = await api.get(`/people/${route.params.id}`);
    } catch (e) {
  ...
  const save = async () => {
    try {
      ...
      if (isNew.value) {
        ...
        s.value = await api.patch(`/people/${s.value._id}`, payload);
        stamp();
      }
    } catch (e) {
  ...
  const destroy = async () => {
    ...
    try {
      await api.del(`/people/${s.value._id}`);
      router.push('/people');
    } catch (e) {
  ...
  const uploadPhoto = async (ev) => {
    ...
    try {
      const updated = await api.post(`/people/${s.value._id}/photo`, fd, {
  ...
  const removePhoto = async () => {
    ...
    try {
      const updated = await api.del(`/people/${s.value._id}/photo`);
      s.value = updated;
      stamp();
    } catch (e) {
  ...
  const searchUsers = async () => {
    try {
      userResults.value = await api.get('/users', userQuery.value ? { q: userQuery.value } : undefined);
    } catch (e) {
