          ...
          <div v-for="r in myList" :key="r._id" class="runsheet card">
            <div class="runsheet__head">
              ...
              <div class="runsheet__actions">
                <RouterLink class="btn btn--ghost" :to="'/runsheets/'+r._id">Open</RouterLink>
                <button
...
const stamp = () => {
  const d = new Date();
  lastUpdated.value = d.toLocaleTimeString();
};

const load = async () => {
  loading.value = true; error.value = '';
  try {
    [openList.value, myList.value] = await Promise.all([
      api.get('/runsheets', { open: 1 }),
      api.get('/runsheets', { assignedToMe: 1 })
    ]);
    stamp();
  } catch (e) {
