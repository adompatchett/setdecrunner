      ...
      <ul class="sup-list" v-else>
        <li v-for="s in suppliers" :key="s._id" class="sup-item card">
          <div class="sup-item__main">
            <RouterLink class="sup-item__name" :to="{ name: 'supplier-edit', params: { id: s._id } }">
              {{ s.name }}
            </RouterLink>
...
const search = () => load();
const goNew = () => router.push({ name: 'supplier-new' });
...
async function initMapIfNeeded() {
  ...
  map = new g.maps.Map(mapEl.value, {
    center: { lat: 43.6532, lng: -79.3832 }, // fallback: Toronto
    zoom: 10,
