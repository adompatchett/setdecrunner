<template>
        ...
        <div v-if="editing" class="modal">
          <div class="modal__card">
            ...
            <div class="modal__foot">
              <a v-if="editing.lat && editing.lng" class="link" :href="mapsUrl(editing.lat, editing.lng)" target="_blank" rel="noopener">
                Open in Google Maps
              </a>
              <span class="spacer"></span>
  ...
  /** IMPORTANT: remember the last selected place_id and keep it synced to the draft */
  ...
  function getDetailsById(placeId) {
    return new Promise((resolve) => {
      ...
      ensurePlacesService().getDetails(
        {
          placeId,
          fields: ['name','formatted_address','geometry','formatted_phone_number','website','place_id']
        },
        (res, status) => {
  ...
  async function enrichWithDetails(place) {
    ...
    // make sure global memory and current draft both have the id
  ...
  /** Bind (or rebind) Autocomplete to the current editor input element */
  function bindEditorAutocomplete() {
    ...
    editorAC.addListener('place_changed', async () => {
      ...
      // If we are editing an existing doc, switch to a NEW draft to avoid overwrite
  ...
  async function setupGoogle() {
    try {
      ...
      // editor autocomplete is bound lazily when the modal opens
    } catch {
      gmReady.value = false; // UI still works without Google
    }
  }
  /** --------------------------------------------------------------------- **/
  ...
  const load = async () => {
    ...
    try {
      list.value = await api.get('/places', { q: q.value });
    } catch (e) {
      error.value = e?.response?.data?.error || e.message || 'Failed to load places';
    } finally {
      loading.value = false;
    }
  };
  ...
  const blankPlace = () => ({
    ...
    notes: '',
    photos: [],
    googlePlaceId: '' // will be removed if still empty before POST/PATCH
  });
  ...
  async function applyToolbarSelection() {
    ...
    const details = await enrichWithDetails(toolbarSelected.value);
    const mustStartNew = !!(editing.value && editing.value._id && !creatingNew.value);
  
    if (!editing.value || mustStartNew) {
      creatingNew.value = true;
      editing.value = { ...blankPlace(), ...details };
      await nextTick();
      bindEditorAutocomplete();
    } else {
      Object.assign(editing.value, details);
    }
  ...
  async function applyEditorSelection() {
    if (!editorSelected.value || !editing.value) return;
  ...
  /** Ensure the googlePlaceId is included, but never as an empty string */
  function normalizePayloadForSave(src) {
    const payload = { ...src };
  
    // If we remembered a place_id but payload lacks it, add it
    if (!payload.googlePlaceId && lastGooglePlaceId.value) {
      payload.googlePlaceId = lastGooglePlaceId.value;
    }
    // If it’s still empty string, drop it so server treats as absent
    if (!payload.googlePlaceId || payload.googlePlaceId === '') {
  ...
  const save = async () => {
    ...
    try {
      if (creatingNew.value) {
        ...
        // POST once; server will return existing if same googlePlaceId (idempotent)
        editing.value = await api.post('/places', payload);
        creatingNew.value = false;
      } else {
        const payload = normalizePayloadForSave(editing.value);
        editing.value = await api.patch(`/places/${editing.value._id}`, payload);
      }
      await load();
    } catch (e) {
      error.value = e?.response?.data?.error || 'Failed to save place';
    } finally {
      saving.value = false;
    }
  };
  ...
  const del = async (p) => {
    ...
    try {
      await api.del(`/places/${p._id}`);
      await load();
    } catch (e) {
      error.value = e?.response?.data?.error || 'Failed to delete place';
    }
  };
  
  // Photos (added in modal)
  const uploadPhotos = async (e) => {
    ...
    try {
      const updated = await api.post(
        `/places/${editing.value._id}/photos`,
        fd,
        { headers: { 'Content-Type': 'multipart/form-data' } }
      );
      editing.value = updated;
      await load(); // refresh listing thumbs
    } catch (e2) {
  ...
  const removePhoto = async (url) => {
    try {
      const updated = await api.del(`/places/${editing.value._id}/photos`, { url });
      editing.value = updated;
      await load();
    } catch (e) {
  ...
  // Import from Google Place ID (server-side details)
  const importFromGoogle = async () => {
    if (!importPlaceId.value.trim()) return;
    importing.value = true; error.value = '';
    try {
      const p = await api.post('/places/import', { googlePlaceId: importPlaceId.value.trim() });
      importPlaceId.value = '';
      list.value.unshift(p);
      await openEditor(p);
    } catch (e) {
      error.value = e?.response?.data?.error || 'Failed to import place';
    } finally {
  ...
  const closeItemsAt = () => {
    itemsAt.value = null; itemsList.value = []; itemQ.value = '';
    quickItemName.value = ''; quickItemQty.value = 1; itemsError.value = '';
  };
  const loadItemsAt = async () => {
    ...
    try {
      ...
      itemsList.value = await api.get('/items', params);
    } catch (e) {
      itemsError.value = e?.response?.data?.error || 'Failed to load items at this place';
    } finally {
  ...
  const quickAddItem = async () => {
    ...
    try {
      ...
      itemsError.value = e?.response?.data?.error || 'Failed to add item';
    } finally {
  ...
  const detachItem = async (it) => {
    try {
      await api.patch(`/items/${it._id}`, { location: null });
      await loadItemsAt();
    } catch (e) {
      itemsError.value = e?.response?.data?.error || 'Failed to remove item from location';
    }
  };
