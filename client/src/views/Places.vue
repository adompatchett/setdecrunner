...
async function setupGoogle() {
  try {
    ...
    await initMapIfNeeded();     // <-- init map once Google is ready
    updateMarkers();             // <-- and plot existing places
  } catch {
...
function extractLatLng(p) {
  ...
  // Fallback: GeoJSON { type:'Point', coordinates:[lng,lat] }
...
async function initMapIfNeeded() {
  ...
  map = new window.google.maps.Map(mapEl.value, {
    center: { lat: 43.6532, lng: -79.3832 }, // default
    zoom: 10,
...
onMounted(async () => {
  ...
  await load();
  if (gmApiKey) await setupGoogle(); // initializes Google + map + markers
  else await initMapIfNeeded();      // if no key was set earlier, still try map init (noop without Google)
});
